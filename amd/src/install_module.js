            ...
            var sectionModules = function() {
                $('.template_section').change(function() {
                    ...
                    if ($('input:checked').length !== 0) {
                        ...
                        var getTheString = str.get_string('select_section_after', 'block_content_templates');
                        $.when(getTheString).done(function(theString) {
                            $('#target_topic_btn').html(theString);
                        });
                        // Change button title to section text
                        var getTheString = str.get_string('select_section_after_mouseover', 'block_content_templates');
                        $.when(getTheString).done(function(theString) {
                        ...
                        var getTheString = str.get_string('select_section', 'block_content_templates');
                        $.when(getTheString).done(function(theString) {
                            $('#target_topic_btn').html(theString);
                        });
                        // Change button mouseover to module text
                        var getTheString = str.get_string('select_section_mouseover', 'block_content_templates');
                        $.when(getTheString).done(function(theString) {
                            $('#target_topic_btn').attr('title', theString);
                        });
                    }
...
// ---------------------------------------------------------------------------------------------------------------------
            var execute = function () {
                $(".content_templates-sections .dropdown-item").on('click', function () {

                    $('.content_templates-modal').show(); // Show the graphical interlude...
...
// ---------------------------------------------------------------------------------------------------------------------
            var callAjax = function(data) {
                var execUrl = config.wwwroot + '/blocks/content_templates/execute.php';
                $.ajax({
                    url: execUrl,
                    type: "POST",
                    data: {
                    ...
                    success: function (result) {
                        $('#content_templates-modal-msg').html(result);
                    },
                    error: function (e) {
                        $('.content_templates-modal').hide();
                        console.error(e);
                    }
                });
            }
...
// _____________________________________________________________________________________________________________________
            $(document).ready(function () {
                ...
                $('#content_templates-spinner-modal').hide();
