...
class block_content_templates extends block_base {
...
//----------------------------------------------------------------------------------------------------------------------
    function init() {
        $this->title = get_string('pluginname', 'block_content_templates');
    }
...
//----------------------------------------------------------------------------------------------------------------------
    function get_content() {
        ...
        if (!$this->page->user_is_editing()) {
            ...
            $PAGE->requires->js_call_amd('block_content_templates/install_module', 'init', array());
...
//----------------------------------------------------------------------------------------------------------------------
    function get_library_modules() {
        ...
        if(!$lib_course_id) {
            $lib_course_id = get_config('block_content_templates', 'defaulttemplate');
         }
        ...
        if(sizeof($sections) == 0) {
            return get_string('no_library', 'block_content_templates');
        }
...
//----------------------------------------------------------------------------------------------------------------------
    function render_modules($sections) {
        ...
        $o .= html_writer::start_tag('div',array('id' => 'content_templates-spinner-modal', 'class' => 'content_templates-modal', 'style' => 'display: none;'));
        ...
        $o .= html_writer::tag('div',get_string('please_wait', 'block_content_templates'), array('id' => 'content_templates-modal-msg', 'style' => 'margin-top: 10px;'));
        $o .= html_writer::end_div();
        $o .= html_writer::end_div();

        // An introduction
        $o .= html_writer::div(get_string('intro_text', 'block_content_templates'));
        ...
        foreach($sections as $section) {
            ...
            if($section->name == '') {
                $section_name = get_string('generic_sectionname', 'block_content_templates') . ' '. $section->section;
            } else {
    ...
    function render_modules0($raw_mods) {
        ...
        $o = '';
        // create a modal dialog that will be shown when installing modules
        $o .= '<div class="content_templates-modal" style="display: none;" id="content_templates-spinner-modal">';
        $o .= '<div class="spinner-container">';
        $o .= '<img src="https://localhost/moodle/theme/image.php/boost/core/1569403484/i/loading" class="spinner">';
        $o .= '<div id="content_templates-modal-msg" style="margin-top: 10px;">'.get_string('please_wait', 'block_content_templates').'</div>';
        $o .= '</div></div>';
...
//----------------------------------------------------------------------------------------------------------------------
    function build_topics_menu() {
        ...
        $title = get_string('select_section_mouseover', 'block_content_templates');
        ...
        $o .= get_string('select_section', 'block_content_templates');
        $o .= html_writer::end_tag('button');

        $o .= html_writer::start_tag('div', array('class' => 'dropdown-menu content_templates-sections'));
        foreach($coursesections as $section) {
            if($section->name == '') {
                $section_name = get_string('generic_sectionname', 'block_content_templates') . ' ' . $section->section;
            } else {
                $section_name = $section->name .' ('.get_string('generic_sectionname', 'block_content_templates') . ' ' . $section->section .')';
            }
